...
import { MdDashboard, MdGpsFixed, MdGpsOff, MdNavigateNext } from "react-icons/md";
import {
...
function StudentDashboard() {
  ...
  // FIX 1: Create socket INSIDE useEffect so it's stable and doesn't recreate on re-render
  ...
  // FIX 2: Initialize socket once on mount, clean up on unmount
  useEffect(() => {
    const s = io(SERVER, {
    ...
    s.on("connect", () => {
      console.log("✅ Student socket connected:", s.id);
  ...
  useEffect(() => {
    ...
    axios
      .get(`${SERVER}/api/student/dashboard`, {
  ...
  // FIX 3: Join bus room ONLY after data is loaded (busId available) and socket is ready.
  // This was the main bug — the old code ran before data was fetched so busId was undefined.
  useEffect(() => {
    const s = socketRef.current;
    const busId = data?.bus?._id;

    // Don't proceed if socket not initialized or busId not available
    if (!s || !busId) return;

    const room = String(busId).trim();

    const joinRoom = () => {
      console.log("📡 Joining bus room:", room);
      s.emit("joinBusRoom", room);
    };
    ...
    const onRoomJoined = (d) => {
      console.log("✅ Room confirmed:", d.room, "| expected:", room);
    };

    const onLiveLocation = (loc) => {
      if (!loc) { console.warn("❌ loc is null"); return; }

      console.log("📍 liveLocation received:", JSON.stringify(loc));

      // FIX 4: Robust room matching — both sides must be trimmed strings
      if (String(loc.busId).trim() !== room) {
        console.warn(`❌ Room mismatch: got [${loc.busId}] expected [${room}]`);
        return;
      }
      ...
      if (isNaN(lat) || isNaN(lng)) {
        console.warn("❌ lat/lng NaN:", loc.lat, loc.lng);
        return;
      }
      // Skip the initial 0,0 placeholder set when trip starts
      if (lat === 0 && lng === 0) {
        console.warn("⚠️ Skipping 0,0 — no GPS yet");
        return;
      }

      console.log(`✅ Bus location updated: [${lat}, ${lng}]`);
    ...
    const onTripStarted = (d) => {
      console.log("🟢 tripStarted:", d);
      setTripActive(true);
      pushNotif("🚌 Bus trip started!", "success");
    };

    const onTripEnded = () => {
      console.log("🛑 tripEnded");
      setTripActive(false);
      setBusLocation(null);
      setEta(null);
      setLocationAge(null);
  ...
  }, [data, pushNotif]); // re-runs when data loads (so busId is available)

  // FIX 5: Check if trip already active on page load (for page refresh recovery)
  useEffect(() => {
    const busId = data?.bus?._id;
    if (!busId || !token) return;

    axios
      .get(`${SERVER}/api/student/active-trip/${busId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
        if (res.data.active) {
          ...
          if (lat != null && lng != null) {
            ...
            if (!isNaN(parsedLat) && !isNaN(parsedLng) && !(parsedLat === 0 && parsedLng === 0)) {
              setBusLocation([parsedLat, parsedLng]);
              setLocationAge(Date.now());
            }
          }
  ...
  return (
    <div className="sd-root">
                  ...
                  {busLocation ? (
                    ...
                    <>
                      ...
                      <div className="sd-eta-lbl">
                        {tripActive ? "Bus location updating…" : "Waiting for bus to start…"}
                      </div>
                    </>
                  )}
                </div>
                ...
                >
                  ...
                  {busLocation && (
                    <Marker position={busLocation} icon={busIcon}>
                      <Popup>
                        🚌 {bus?.busNumber}<br />
                        {busLocation[0].toFixed(5)}, {busLocation[1].toFixed(5)}
                      </Popup>
                    </Marker>
                  )}
                </MapContainer>
                ...
                <>
                  ...
                  <div className="sd-card">
                    ...
                    <div className="sd-stops-list">
                      {route.stops?.map((s, i) => (
                        <div key={i} className={`sd-stop-item ${s.name?.trim().toLowerCase() === data?.stopName?.trim().toLowerCase() ? "mine" : ""}`}>
                  ...
                  <div className="sd-card">
                    ...
                    <div className="sd-timings-grid">
                      {route.timings?.map((t, i) => (
                        <div className="sd-timing-chip" key={i}><FaClock /> {t}</div>
                      ))}
                    </div>
                  </div>
                </>
              ) : (
                <div className="sd-empty-state"><FaRoute /><p>No route assigned yet</p><span>Contact your administrator</span></div>
              )}
            </div>
          )}
      ...
      <nav className="sd-bottom-nav">
        {navItems.map((n) => (
          <button key={n.id} className={`sd-bottom-btn ${activeTab === n.id ? "active" : ""}`} onClick={() => setActiveTab(n.id)}>
            {n.icon}<span>{n.label}</span>
          </button>
        ))}
      </nav>
